...
const MintPresenter = () => {
  ...
  const uploadImage = async () => {
    ...
    const img = await NftcreateAPI.sendIpfs(formData);

    const metadata = {
      title: "test",
      description: "test",
      media: `https://${img.value.cid}.ipfs.nftstorage.link`,
    };
    const result = await mint("token-177", metadata);
    console.log(result);
    return result;
  };

  const mint = async (tokenId, metadata) => {
    ...
    // NEAR 계정 연결
    const account = await nearConnection.account("ismarket.testnet");
    console.log(account);
    const tokenMetadata = {
      token_id: tokenId,
      metadata: metadata,
      receiver_id: "ismarket.testnet",
      perpetualRoyalties: {
        "ed25519:7jctNXAsYADk3M96dCwhPv2jL6yQgSEo9G6r8BDXR7ps": 0.1,
      },
    };
    ...
    const contract = new Contract(
      ...
      "ismarket.testnet",
      {
        ...
        changeMethods: ["nft_mint"], // view methods do not change state but usually return a value
        sender: "ismarket.testnet",
      }
    );
    // mintNFT 메서드 호출
    const result = await contract.nft_mint(tokenMetadata); // tokenMetadata는 발행할 NFT의 메타데이터를 포함하는 객체
    console.log(result); // 호출 결과 출력
    return result;
  };
          ...
          <div className="pt-32 pb-12 md:pt-40 md:pb-20">
            <div className="pb-12 md:pb-14">
              <div className="relative text-center md:text-left">
                ...
                <h2 className="h2 font-cabinet-grotesk mb-12">NFT Mint</h2>
              </div>
